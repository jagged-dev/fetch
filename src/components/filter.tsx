...
export default function Filter() {
    ...
    function toggleBreed(breed: string, checked: boolean) {
        if (checked) setBreeds({ ...breeds, selected: [...breeds.selected, breed] });
        else setBreeds({ ...breeds, selected: breeds.selected.filter((item: string) => item !== breed) });
    }
                    ...
                    <DialogPanel transition className="flex h-full w-full flex-col gap-8 overflow-y-auto rounded-lg bg-snow p-8 transition data-[closed]:scale-95 data-[closed]:opacity-0 dark:bg-coal">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                                <button onClick={() => setBreeds({ ...breeds, selected: [] })} disabled={breeds.selected.length === 0} className="group">
                        ...
                        <div className="flex flex-col gap-2">
                            <p>Age</p>
                            <div className="flex gap-2 rounded-lg border border-gunmetal/25 p-2 hover:border-gunmetal/50 dark:border-silver/25 dark:hover:border-silver/50">
